  ...
  <q-card class="q-pa-sm">
    <q-scroll-area class="q-pa-sm" style="height: 80vh">
      ...
      <div class="row items-center q-gutter-md q-mb-md">
        ...
        <div class="col-auto">
          <!-- Structure button - only if user has report permission -->
          <q-btn
            v-if="canReportPlantilla"
            dense
            color="primary"
            label="Structure"
            icon="print"
            ...
            size="sm"
            @click="handlePrintStructure"
          />
          <!-- Request for Publication button - only if user has request publication permission -->
          <q-btn
            v-if="canRequestPublication"
            dense
            color="primary"
            label="Request for Publication"
            icon="print"
            class="q-mb-sm"
            size="sm"
            @click="handleRequestPublication"
          />
        </div>
      </div>

      <!-- DBM Print Dialog -->
      <q-dialog v-model="showDBMDialog" persistent>
        <PlantillaReport @close="showDBMDialog = false" />
      </q-dialog>

      <!-- Structure Print Dialog -->
      <q-dialog v-model="showStructureDialog" persistent>
        <PlantillaReport @close="showStructureDialog = false" />
      </q-dialog>

      <!-- Request for Publication Modal -->
      <RequestPublicationModal
        v-if="showPublicationModal"
        @close="showPublicationModal = false"
...
<script setup>
  ...
  import RequestPublicationModal from 'src/components/RequestPublicationModal.vue';
  ...
  // Permission checks
  const canRequestPublication = computed(() => {
  ...
  const handleGenerate = (selectedPositions) => {
    // Handle the generated publication request
    console.log('Publication request generated:', selectedPositions);
  };
  ...
  const generateTreeStructure = (items, officeName) => {
    ...
    items.forEach((item) => {
      ...
      if (hasOffice2) {
        ...
        if (hasGroup) {
          ...
          if (hasDivision) {
            if (!group.divisions.has(item.division)) {
              group.divisions.set(item.division, { sections: new Map(), directUnits: new Map() });
            }
            const division = group.divisions.get(item.division);

            if (hasSection) {
              if (!division.sections.has(item.section)) {
                division.sections.set(item.section, { units: new Map() });
              }
              if (hasUnit) division.sections.get(item.section).units.set(item.unit, {});
            } else if (hasUnit) {
              division.directUnits.set(item.unit, {});
            }
          } else if (hasSection) {
            if (!group.directSections.has(item.section)) {
              group.directSections.set(item.section, { units: new Map() });
            }
            if (hasUnit) group.directSections.get(item.section).units.set(item.unit, {});
          } else if (hasUnit) {
          ...
          if (!office2.directSections.has(item.section)) {
            office2.directSections.set(item.section, { units: new Map() });
          }
          if (hasUnit) office2.directSections.get(item.section).units.set(item.unit, {});
        } else if (hasUnit) {
        ...
        if (!structure.directSections.has(item.section)) {
          structure.directSections.set(item.section, { units: new Map() });
        }
        if (hasUnit) structure.directSections.get(item.section).units.set(item.unit, {});
      } else if (hasUnit) {
    ...
    const createSectionNode = (sectionName, parentData, sectionData) => {
      ...
      sectionData.units.forEach((_, unitName) => {
        node.children.push(createUnitNode(unitName, node.data));
      });
      return node;
    };
    ...
    const createDivisionNode = (divisionName, parentData, divisionData) => {
      ...
      divisionData.sections.forEach((sectionData, sectionName) => {
        node.children.push(createSectionNode(sectionName, node.data, sectionData));
      });
    ...
    const createGroupNode = (groupName, parentData, groupData) => {
      ...
      groupData.divisions.forEach((divisionData, divisionName) => {
        node.children.push(createDivisionNode(divisionName, node.data, divisionData));
      });
