  ...
  function processApplicants(applicants, type, targetArray) {
    ...
    applicants.forEach((item) => {
      ...
      if (applications.length === 0) {
        targetArray.push({
          applicantName: formatFullName(applicant.firstname, applicant.lastname),
          position: 'N/A',
          office: 'N/A',
          salaryGrade: 'N/A',
          itemNo: 'N/A',
          email: (applicant.email_address || '').toLowerCase(),
          contactNumber: cleanPhoneNumber(applicant.cellphone_number),
          rowspan: 1,
          isFirstRow: true,
          type: type,
        });
  ...
  async function generatePdfContent() {
    ...
    import('pdfmake/build/pdfmake').then((pdfMakeModule) => {
      ...
      import('pdfmake/build/vfs_fonts').then((vfsFontsModule) => {
        ...
        // Helper function to add applicant rows for a section with page break border handling
        function addSectionRows(rows, sectionTitle) {
          if (rows.length === 0) return;

          // Add section header row - grey background (same as column header)
          tableBody.push([
            {
              text: sectionTitle,
              colSpan: 7,
          ...
          while (currentIndex < rows.length) {
            ...
            if (rowspan > 1 && row.isFirstRow) {
              ...
              // Add custom border properties to handle page breaks
              tableBody.push([
                {
                  text: row.applicantName || '',
                  rowSpan: rowspan,
                  alignment: 'left',
                  border: [true, true, true, false], // bottom border will be handled by page break detection
                },
                {
                  text: row.position || '',
                  alignment: 'left',
          ...
          content: [
            ...
            },
            {
              ...
              layout: {
                ...
                // Custom border function to handle rowspan page breaks
                hLineWidth: function () {
                  return 0.3;
                },
                vLineWidth: function () {
