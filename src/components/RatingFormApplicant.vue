            ...
            <table class="rating-table">
              ...
              <thead class="sticky-criteria">
                <tr>
                  ...
                  <!-- Exam always last before Grand Total -->
                  <th v-if="hasExam">
                    Exam
...
<script setup>
  ...
  const education = computed(() => {
    ...
    return { Rate: edu.weight || edu.Rate || '20', items: edu.items || [] };
  });

  const experience = computed(() => {
    const exp = props.criteria.experience || {};
    return { Rate: exp.weight || exp.Rate || '20', items: exp.items || [] };
  });

  const training = computed(() => {
    const train = props.criteria.training || {};
    return { Rate: train.weight || train.Rate || '15', items: train.items || [] };
  });

  const performance = computed(() => {
    const perf = props.criteria.performance || {};
    return { Rate: perf.weight || perf.Rate || '15', items: perf.items || [] };
  });

  const behavioral = computed(() => {
    const beh = props.criteria.behavioral || {};
    return { Rate: beh.weight || beh.Rate || '0', items: beh.items || [] };
  });
  ...
  const getMinRate = (items) => {
    if (!items || items.length <= 1) return null;
    const mins = items.map((i) => parseFloat(i.percentage)).filter((n) => !isNaN(n));
    if (!mins.length) return null;
    return Math.min(...mins);
  };

  const educationMinRate = computed(() => getMinRate(education.value.items));
  const experienceMinRate = computed(() => getMinRate(experience.value.items));
  const trainingMinRate = computed(() => getMinRate(training.value.items));
  const performanceMinRate = computed(() => getMinRate(performance.value.items));
  const behavioralMinRate = computed(() => getMinRate(behavioral.value.items));
  ...
  const totalMaxRate = computed(
    () =>
      qsMaxRate.value +
      (hasBehavioral.value ? behavioralMaxRate.value : 0) +
  ...
  // Dynamic column widths based on longest criteria description text
  const COL_MIN = 180;
  const COL_MAX = 500;
  ...
  const isValidScore = (score, allowEmpty = false) => {
    if (isEmpty(score)) return allowEmpty;
  ...
  const validateScore = (applicant, field, maxValue, allowEmpty = false, minValue = null) => {
    ...
    if (minValue !== null && numValue < minValue) {
      $q.notify({
        color: 'warning',
        message: `Score cannot be less than ${minValue}`,
        icon: 'warning',
        position: 'top',
        timeout: 2000,
      });
      applicant[field] = formatNumber(minValue);
      calculateAllRankings();
      return;
    }
