                  ...
                  <q-scroll-area style="height: calc(100% - 28px); min-height: 100px">
                    ...
                    >
                      ...
                      <template v-slot:body-cell-attachment="props">
                        <q-td :props="props" class="text-center">
                          <q-btn
                            v-if="props.row.attachment || props.row.attachment_url"
                            icon="attach_file"
                            size="sm"
                            color="primary"
                            flat
  ...
  >
    ...
    >
      ...
      >
        ...
        <q-space />
        <q-btn icon="close" flat round dense @click="showFileViewer = false" />
      </q-card-section>
      ...
      >
        ...
        <div v-else class="column items-center q-gutter-md">
          ...
          <div class="text-h6 text-grey">File type not supported for preview</div>
          <q-btn label="Download File" color="primary" icon="download" @click="downloadFile" />
        </div>
      </q-card-section>

      <!-- Footer Actions -->
      <q-card-section class="row justify-end q-py-sm bg-grey-2" style="flex-shrink: 0">
        <q-btn
          v-if="fileUrl"
          label="Download"
          icon="download"
          ...
          @click="downloadFile"
        />
        <q-btn label="Close" color="grey" flat @click="showFileViewer = false" />
      </q-card-section>
    </q-card>
  </q-dialog>
...
<script setup>
  ...
  const viewAttachment = async (row) => {
    try {
      ...
      let url = row.attachment_url || row.attachment || row.attachment_path;
      if (!url) {
